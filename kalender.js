...
(function () {
  ...
  function render() {
    ...
    updateStats();
    renderAttendance(); // juga akan memicu cek milestone & bunyi jika perlu
  }
  ...
  // ====== Audio Celebrate ======
  // Menggunakan Web Audio API, aman untuk autoplay policy karena dipicu dari aksi klik (checkbox).
  let audioCtx = null;
  function ensureAudioCtx() {
    ...
    // Fanfar mini: arpeggio C–E–G + drum hit sederhana
  ...
  // ====== Absen 100 (cek & bunyi di milestone) ======
  function renderAttendance() {
    ...
    for (let i = 0; i < ATTENDANCE_SIZE; i++) {
      ...
      cb.addEventListener('change', e => {
        ...
        saveAbsen();
        // Render ulang akan menghitung 'done' baru dan memutuskan apakah perlu bunyi
        renderAttendance();
      });
      td.appendChild(cb);
    ...
    // === TRIGGER SOUND: hanya ketika mencapai kelipatan 10 yang baru ===
    if (done > 0 && done % 10 === 0 && done !== lastCelebrated) {
    // Bunyi dulu
    playCelebrateSound();

    // Lalu popup SweetAlert
    ensureSwal().then((Swal) => {
        ...
        // Fallback kalau CDN gagal
        alert(`Congrats! Strike ${done} tercapai (${label}).`);
        }
    });

    // Tandai agar tidak bunyi/popup berulang
    lastCelebrated = done;
    saveCelebrated(done);
    }
